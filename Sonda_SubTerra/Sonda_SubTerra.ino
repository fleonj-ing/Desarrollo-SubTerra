...
ESP32 --- SONDA
--------------------------------------------------------------------------------
Este archivo implementa el comportamiento de la sonda:
1) Gestiona sensores de contacto/medio (agua y tierra).
2) Controla motor de barrido y detección de referencia por CNY70.
3) Realiza lectura confiable del sensor de distancia VL53L1X.
4) Atiende comandos de la interfaz y devuelve telemetría/eventos.
5) Reporta estado energético mediante LED RGB según batería.

Interacción con Interfaz_MOD11:
- Recibe comandos de modo, inicio/cancelación de escaneo y resolución.
- Envía estado de enlace, disponibilidad de sensor, lecturas VL53 y alertas.
- Informa tiempos de inicio/fin de barrido para trazabilidad de campaña.
================================================================================
*/
//  Liberias y Pines
...
void setup() {
  /*
    Arranque de la sonda:
    - Inicializa monitor de batería, motor y entradas de sensores.
    - Levanta comunicación serie con la interfaz.
    - Configura interrupción de marca de vuelta (CNY70).
    - Intenta iniciar VL53L1X para dejar el nodo listo para escaneo.
  */
  initBattery();
  motorBegin();
  pinMode(SAS,INPUT);
...
void loop() {
  /*
    Lazo principal:
    - Drena comandos entrantes y aplica efectos semánticos.
    - Mantiene actualizado el LED de batería.
    - Prioriza eventos de seguridad (agua/tierra).
    - Atiende dos tareas remotas: diagnóstico VL53 e inicio de escaneo.
  */
...
bool initVL53() {
  // Configura dirección, modo de distancia y timing del sensor ToF.
...
bool readReliableDistance(int16_t &mm, uint32_t &t_us, uint8_t &rangeStatus) {
  /*
    Filtro de calidad de lectura:
    - Solo procesa cuando hay dato listo.
    - Acepta status de rango válido.
    - Descarta valores físicamente improbables para evitar ruido.
  */
...
bool readIncomingPayload(SubTerraComms::DecodedPayload& payload) {
  // Extrae un payload semántico de la cola de bytes UART (si existe).
  SubTerraComms::ParseResult parseStatus = SubTerraComms::PARSE_INCOMPLETE;
  return comms.pollPayload(payload, &parseStatus);
}
void applyPayloadEffects(const SubTerraComms::DecodedPayload& payload) {
  /*
    Mapeo de comandos recibidos a estado interno de la sonda.
    Aquí se traduce "intención de la interfaz" a flags operativas:
    - iniciar escaneo
    - solicitar estado VL53
    - ajustar resolución/velocidad de barrido
...
void processIncomingSerial() {
  // Bucle de consumo completo de mensajes pendientes en este ciclo.
...
void readCny70() {
  // ISR mínima: marca detección de referencia para terminar el barrido actual.
  cny70Detectado = true;
}
void baterryCharge(){
  /*
    Convierte el porcentaje de batería a código de color RGB:
    verde = alto, amarillo = medio, rojo = crítico.
  */
...
void setColor(int r, int g, int b) {
  // Aplica color al LED RGB usando canales PWM.
...
// Inicialización
  // --------------------------------------------------
  // Inicializa pines del puente H y deja el motor en reposo seguro.
  void motorBegin() {
  ...
  // --------------------------------------------------
  /*
    Traduce velocidad firmada a señales de dirección + PWM.
    Permite unificar control sin duplicar lógica de sentido de giro.
  */
  ...
  // Asigna velocidad de forma inmediata (sin transición por rampa).
