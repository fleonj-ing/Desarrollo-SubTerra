...
/*
  Implementación del protocolo binario robusto SubTerra.
  Este archivo resuelve tres responsabilidades:
  1) Construcción y envío de frames con CRC.
  2) Reconstrucción de frames recibidos (máquina de estados).
  3) Traducción a payload semántico para la lógica de aplicación.
*/
...
void SubTerraComms::begin(uint32_t baud, int8_t rxPin, int8_t txPin) {
  // Abstracción de inicialización serie para ESP32 y Teensy.
#if defined(ESP32)
  if (rxPin >= 0 && txPin >= 0) {
...
uint16_t SubTerraComms::crc16Ccitt(const uint8_t* data, size_t len) {
  // CRC16-CCITT estándar para detección de errores de transmisión.
  uint16_t crc = 0xFFFF;
...
void SubTerraComms::sendFrame(uint8_t type, const uint8_t* payload, uint8_t len) {
  // Empaqueta cabecera + payload + CRC y transmite frame completo.
...
void SubTerraComms::sendResolutionValue(uint8_t resolution_1_to_10) {
  if (resolution_1_to_10 < 1 || resolution_1_to_10 > 10) return;
...
bool SubTerraComms::pollPayload(DecodedPayload& outPayload, ParseResult* outLastParse) {
  // API de alto nivel: retorna solo comandos válidos para la aplicación.
...
SubTerraComms::ParseResult SubTerraComms::readSerialDuplex(DecodedFrame& outFrame) {
  /*
    Parser incremental por estados:
    espera SOF, luego cabecera, payload y CRC.
    Permite operar en polling sin bloquear aunque la trama llegue fragmentada.
  */
