...
================================================================================
DESAROLLO INNOVAR
Nombre clave: Subterra
Fecha de modificacion: 04/16/2026
Autor: Fernando León Jimémez
Teensy 4.1 --- HMI
--------------------------------------------------------------------------------
Este archivo coordina toda la lógica de la estación de superficie:
1) Inicializa pantalla, almacenamiento, comunicaciones y estado de batería.
2) Presenta el menú principal y dirige al operador a cada modo de trabajo.
3) Ejecuta tres estrategias de escaneo (manual, guiado y automático).
4) Registra datos crudos (tiempo, distancia y posición vertical) en SD interna.
5) Intercambia comandos/eventos con la sonda (inicio/fin, agua, tierra, estado).

Interacción con otros módulos:
- SubTerraComms.*: protocolo binario robusto para interfaz↔sonda.
- BatteryMonitor.*: estimación y filtrado de carga de batería.
- Button.*: antirrebote y eventos de pulsación/repetición.
- icons_16x16.h: activos gráficos para estados e iconografía de la UI.

Nota de mantenimiento:
La variable "menu" define el modo visible en pantalla.
La variable "controladoActivo" transfiere el control del loop principal a una
máquina de estados no bloqueante (FSM) para el modo guiado.
================================================================================
*/
//Pines
...
/*Variables Motor
  ...
  */
  ...
  const int PIN_AIN1 = 38;
  const int PIN_AIN2 = 40;
  //const int PIN_STBY = 2; //PENDIENTE
  Encoder myEnc(22, 23);

  // Variable de encoder
  long oldPosition  = -999;
  int dist3Cm = 3000; // PULSOS DEL ENCODER
...
void setup() {
  /*
    ...
    - Inicializa batería y control del motor local.
    - Levanta UART de comunicaciones con la sonda.
    - Configura TFT y buses SPI/SD.
    - Ejecuta autoverificación inicial (SD + VL53 remoto + enlace).
    - Dibuja interfaz base del menú y deja botones listos.
  */
  initBattery();
  motorBegin();
  Serial.begin(9600);
...
void loop() {
  /*
    ...
    - Refresca botones y mensajes entrantes en cada iteración.
    - Si el modo guiado está activo, delega en la FSM de controlado.
    - En modo menú, permite navegar y entrar a una pantalla de operación.
  */
  btnSelect.update();
  btnUp.update();
  btnDown.update();
...
///////////////////////////////////////   Menu
void actualizarMenu() {
  // Redibuja resaltado del menú circular (1..4) sin reconstruir toda la pantalla.
  tft.setTextSize(3);
  switch (menu) {
...
void actualizarPantalla(){
  /*
    Puerta de entrada a los modos.
    Además limpia flags "latched" de corrida anterior para evitar que eventos
    viejos (agua/tierra/fin de escaneo) contaminen la siguiente operación.
  */
  hayAgua = false;
  hayTierra = false;
  hayNuevaLecturaVl53 = false;
...
void esc_manual(){
  /*
    Modo manual:
    - El operador manda subir/bajar en tiempo real.
    - SELECT dispara un escaneo puntual en la posición actual.
    - Mientras hay escaneo activo, cada lectura VL53 se persiste en SD.
    - Si llega agua/tierra, se prioriza seguridad y se frena el movimiento.
  */
  Z = 0;
  cambioModoTitulo();
...
void iniciar_controlado() {
  /*
    Inicializa el asistente de escaneo guiado.
    Carga layout visual, resetea capturas (inicio, final, resolución, barridos)
    y transfiere control al flujo por estados.
  */
...
void fsm_controlado() {
  /*
    Máquina de estados del modo guiado:
    1) Captura profundidad inicial.
    2) Captura profundidad final.
    3) Ajusta resolución.
    4) Ajusta número de barridos.
    5) Ejecuta el plan: posicionamiento, escaneo por tramos y retorno seguro.

    Beneficio: evita un bloque monolítico difícil de mantener y permite
    validaciones por etapa con navegación adelante/atrás.
  */
...
void esc_automatico() {
  /*
    Modo automático:
    - Solicita una profundidad objetivo.
    - Lleva el sistema a referencia superior.
    - Baja a la posición solicitada y encadena escaneos por pasos.
    - Registra mediciones y termina al completar secuencia o por evento de seguridad.
  */
  flagUndo = 0;
  profundidad = 0;
  queDist = 2;
...
void prueba_sistema(){
  /*
    Modo de prueba:
    Banco de validación operativa para actuadores, sensores y comunicaciones.
    Se usa para diagnóstico rápido sin ejecutar una rutina completa de campo.
  */
  // Arranca con velocidad BAJA (muchos módulos fallan a 25MHz)
  SdSpiConfig cfg(SD_CS, DEDICATED_SPI, SD_SCK_MHZ(SD_MHZ), &SPI1);
  cambioModoTitulo();
  ...
  while(true){
    ...
    if (btnSelect.wasPressed()){
      ...
      const uint32_t SCAN_TIMEOUT_MS = 10000; //  AJUSTAR
...
void cambioModoTitulo(){
  // Dibuja encabezado común para todas las pantallas de modo operativo.
...
void valorProf(){
  /*
    Ajustador de profundidad con aceleración de entrada:
    - Pulsación corta: ajuste fino.
    - Pulsación mantenida: incremento/decremento repetitivo.
    Es utilizado por modos guiado y automático para capturar setpoints.
  */
...
void flushBotones(){
  // Sincroniza estados internos de botones para evitar "rebotes lógicos".
  while (true) {
...
bool readIncomingPayload(SubTerraComms::DecodedPayload& payload){
  // Lee exactamente un payload semántico ya validado por CRC/protocolo.
...
void applyPayloadEffects(const SubTerraComms::DecodedPayload& payload){
  /*
    Traduce comandos remotos a flags locales de operación.
    Este desacople evita que el resto del código conozca detalles del parser.
  */
...
void processIncomingSerial(){
  // Drena UART en polling hasta agotar mensajes pendientes del ciclo actual.
...
void moverMotor3Cm(){
  /*
    Movimiento incremental hacia abajo.
    Se usa en escaneo por capas para mantener distancia de paso constante.
  */
  newPosition = 0;
  myEnc.write(0);
...
void moverMotor3CmArriba(){
  // Movimiento incremental hacia arriba, simétrico al avance por capas.
  newPosition = 0;
  myEnc.write(0);
  motorSetTargetSpeed(-255);
...
void saveSdExt(){
  /*
    Exporta archivo crudo interno hacia SD externa.
    Se utiliza al finalizar para respaldo y extracción de datos de campaña.
  */
...
bool crearCrudos(){
  // Crea nombre único de archivo de captura y lo prepara para append de lecturas.
...
void baterryCharge(){
  /*
    Lee porcentaje estimado de batería y actualiza iconografía en la interfaz.
    El valor proviene de BatteryMonitor (promedio + filtro + histéresis).
  */
...
// -----------------------------------------------------------------------------
bool obtenerNombreArchivo(char* nombreFinal, size_t len){
  // Genera nombre incremental (sin colisiones) para cada sesión de escaneo.
...
// Inicialización Motor
  // --------------------------------------------------
  /*
    Inicializa driver puente H y deja el motor en estado seguro de reposo.
  */
  void motorBegin() {
  ...
  // --------------------------------------------------
  /*
    Aplica velocidad instantánea al puente H.
    Convención: positivo = bajar, negativo = subir, cero = paro libre.
  */
  ...
  // --------------------------------------------------
  /*
    Algoritmo de rampa no bloqueante:
    aproxima currentSpeed hacia targetSpeed en pasos fijos cada N milisegundos.
    Reduce picos de corriente y mejora estabilidad mecánica.
  */
